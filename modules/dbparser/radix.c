...
gboolean
r_parser_lnstring(guint8 *str, gint *len, const gchar *param, gpointer state, RParserMatch *match)
{
  guint8 *end;
...
r_new_pnode(guint8 *key)
{
  ...
  else if (strcmp(params[0], "LNSTRING") == 0)
    {
      if (params_len <= 2)
        {
          parser_node->parse = r_parser_lnstring;
          parser_node->type = RPT_LNSTRING;
        }
      else
        {
          g_free(parser_node);
          msg_error("Too many arguments to LNSTRING, no 3rd parameter supported",
                    NULL);
          parser_node = NULL;
        }
    }
