...
get_ip_for_dns_request(void)
{
  ...
  local_hostname = get_local_hostname_from_system();
  ...
  if (local_host)
    {
      ...
      return g_strdup_printf("%d.%d.%d.%d.IN-ADDR.ARPA", addr.S_un.S_un_b.s_b4, addr.S_un.S_un_b.s_b3, addr.S_un.S_un_b.s_b2, addr.S_un.S_un_b.s_b1);
    }
  return NULL;
}
...
get_local_fqdn_hostname_from_dns(void)
{
  ...
  if (has_dns_server())
    {
      gchar *ip_for_dns_request = get_ip_for_dns_request();

      if (ip_for_dns_request)
        {
          status = DnsQuery(
                            ip_for_dns_request,
                            DNS_TYPE_PTR,
